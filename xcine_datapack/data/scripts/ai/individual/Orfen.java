...
public class Orfen extends L2AttackableAIScript
{
	...
	public Orfen(int id, String name, String descr)
	{
		...
		registerMobs(mobs);
		_IsTeleported = false;
		_Zone = GrandBossManager.getZoneByXYZ(Pos[0][0], Pos[0][1], Pos[0][2]);
		...
		if (status == DEAD)
		{
			...
			else
			{
				...
				L2GrandBossInstance orfen = (L2GrandBossInstance) addSpawn(ORFEN, x, y, z, 0, false, 0, false);
	...
	public void spawnBoss(L2GrandBossInstance npc)
	{
		...
		int y = npc.getY();
		L2Attackable mob;
		mob = (L2Attackable) addSpawn(RAIKEL_LEOS, x + 100, y + 100, npc.getZ(), 0, false, 0, false);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("check_minion_loc"))
		{
			for (int i = 0; i < _Minions.size(); i++)
			{
				...
				{
					mob.teleToLocation(npc.getX(), npc.getY(), npc.getZ(), 0);
					((L2Attackable) npc).clearAggroList();
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE, null, null);
				}
			}
		...
		{
			L2Attackable mob = (L2Attackable) addSpawn(RAIKEL_LEOS, npc.getX(), npc.getY(), npc.getZ(), 0, false, 0, false);
			mob.setIsRaidMinion(true);
			_Minions.add(mob);
		}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		else if (GrandBossManager.getBossStatus(ORFEN) == ALIVE && npc.getNpcId() == RAIKEL_LEOS)
		{
			_Minions.remove(npc);
			startQuestTimer("spawn_minion", 360000, npc, null, false);
		}
