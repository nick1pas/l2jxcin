...
public class Q343_UnderTheShadowOfTheIvoryTower extends Quest
{
    ...
    public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
    {
        ...
        else if (event.equals("1") || event.equals("2") || event.equals("3"))
        {
            if (st.getInt("playing") == 1)
            {
                ...
                st.unset("playing");
                htmltext = getHtmlText("30934-04.htm")
                    .replace("%player%", OPTIONS[_player])
                    .replace("%marsha%", OPTIONS[_marsha])
                    .replace("%msg%", msg[Rnd.get(msg.length)])
                    .replace("%again%", AGAIN_MSG[Rnd.get(AGAIN_MSG.length)]);
            }
            else
            {
        ...
        else if (event.equals("4") || event.equals("5"))
        {
            ...
            if (st.getInt("toss") == 1)
            {
                if (st.getQuestItemsCount(NEBULITE_ORB) >= 10)
                {
                    ...
                    htmltext = getHtmlText(htmltext)
                        .replace("%toss%", TOSS[_rnd])
                        .replace("%msg1%", TOSS_MSG[row][0])
                        .replace("%msg2%", TOSS_MSG[row][1])
                        .replace("%orbs%", Integer.toString(ORBS[row]))
                        .replace("%next%", Integer.toString(ORBS[row]));
                }
                else
                {
                    st.unset("row");
    ...
    public static boolean isValidClass(L2PcInstance player)
    {
        if (player.getClassId().isMage() && player.getClassId().getRace() != Race.Orc && player.getClassId().level() >= 1 && !player.isSubClassActive())
            return true;
