...
import net.xcine.gameserver.model.actor.L2Attackable.AbsorberInfo;
...
public class Q350_EnhanceYourWeapon extends Quest
{
    ...
    private void load()
    {
        try
        {
            ...
            File file = new File("./data/scripts/quests/Q350_EnhanceYourWeapon/data.xml");
            if (!file.exists())
            {
    ...
    {
        QuestState st = player.getQuestState(qn);
        if (st == null || st.getState() != Quest.STATE_STARTED)
            return null;
    ...
    private void exchangeCrystal(L2PcInstance player, L2Attackable mob, int takeid, int giveid, boolean broke)
    {
        ...
        if (Item != null)
        {
            ...
            // Send system message
            SystemMessage sms = new SystemMessage(SystemMessageId.EARNED_ITEM_S1);
