...
public class L2SiegeGuardAI extends L2AttackableAI
{
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		...
		if (intention == AI_INTENTION_IDLE)
		{
			...
			if (!_actor.isAlikeDead())
			{
				...
				if (!getActiveChar().getKnownList().getKnownPlayers().isEmpty())
	...
	protected void thinkAttack()
	{
		...
		final String clanName = actor.getClan();
		...
		for (L2SiegeGuardInstance cha : actor.getKnownList().getKnownTypeInRadius(L2SiegeGuardInstance.class, clanRange))
		{
			...
			if (!clanName.equals(cha.getClan()))
				continue;
			
			if (cha.getAI()._intention == CtrlIntention.AI_INTENTION_IDLE || cha.getAI()._intention == CtrlIntention.AI_INTENTION_ACTIVE)
			{
		...
		if (willCastASpell())
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				for (L2Character cha : actor.getKnownList().getKnownTypeInRadius(L2Character.class, 1000))
				{
					...
					if (!actor.isAttackingDisabled() && (cha instanceof L2PcInstance && actor.getCastle().getSiege().checkIsDefender(((L2PcInstance) cha).getClan())) || (cha instanceof L2Npc && clanName.equals(((L2Npc) cha).getClan())))
					{
