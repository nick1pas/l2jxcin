...
public abstract class L2Skill implements IChanceSkillTrigger
{
		...
		switch (_targetType)
		{
			...
			case TARGET_CLAN:
			{
				...
				else if (activeChar instanceof L2Npc)
				{
					...
					targetList.add(activeChar);
					for (L2Npc newTarget : activeChar.getKnownList().getKnownType(L2Npc.class))
					{
						if (npc.getClan().equals(newTarget.getClan()))
						{
			...
			case TARGET_AREA_UNDEAD:
			{
				...
				if (_castRange >= 0 && (target instanceof L2Npc || target instanceof L2SummonInstance) && target.isUndead() && !target.isAlikeDead())
				{
					cha = target;
				...
				for (L2Character obj : activeChar.getKnownList().getKnownType(L2Character.class))
				{
					...
					if (!Util.checkIfInRange(_skillRadius, cha, obj, true))
						continue;
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		else if (caster instanceof L2Attackable && target instanceof L2Attackable)
		{
