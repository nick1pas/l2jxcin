 ...
 */
package net.xcine.gameserver.util;
...
public final class Util
{
	...
	/**
	 * Return the number of players in a defined radius. Dead players aren't counted.<br>
	...
	{
		int count = 0;
		for (L2PcInstance player : npc.getKnownList().getKnownPlayers())
		{
			if (player.isDead())
				continue;
    ...
    public static String scrambleString(String string)
    {
        ...
        String c;
        for(Iterator<String> i$ = letters.iterator(); i$.hasNext(); sb.append(c))
            c = i$.next();
